...
export const regexpCases: Record<string, RegExpCase> = {
  // Lookup ignores up to two trailing slashes (#200): `findRoute` strips one and
  // `splitPath` pops one trailing empty segment. A third leaves a real empty
  // last segment, which only a param can take.
  "/path": {
    regex: /^\/path(?:\/\/|(?<!\/)\/?)$/,
  ...
  "/path/:param": {
    ...
    match: [
      ...
      ["/path/value/", { param: "value" }],
      // The regex used to reject this path while the router routed it (#200).
      ["/path/value//", { param: "value" }],
      ["/path///", { param: "" }],
    ],
