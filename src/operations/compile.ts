 ...
 * @param n
 */
const _hasChild = (n: Node<any>): boolean => n.static != null || n.param != null || n.wildcard != null;

// Skip a native call for common methods
const _fastMethodStringify = (m: string) => m === 'GET'
  ? '"GET"'
  : m === 'POST'
    ? '"POST"'
...
export const _compileMethodMatch = (methods: Record<string, MethodData<any>[] | undefined>, deps: any[]): string => {
  let str = '';
  for (const key in methods) {
    ...
    if (data != null && data.length > 0) {
      ...
      if (key !== '')
        str += `if(m===${_fastMethodStringify(key)})`;
...
export const _compileKeyMatch = (key: string, startIdx: string, ifBody: string): string => `if(p.startsWith(${
  JSON.stringify(key)
}${
  startIdx === '0' ? '' : ',' + startIdx
})){${ifBody}}`;
 ...
 */
export const _compileNode = (notParamNode: boolean, node: Node<any>, idxPrefix: string, startIdx: number, paramCnt: number, deps: any[]): string => {
  let str = '';
  ...
  if (node.static != null)
    for (const key in node.static)
      str += _compileKeyMatch(
        '/' + key, currentIdx,
        _compileNode(true, node.static[key], idxPrefix, startIdx + key.length + 1, paramCnt, deps)
      );
  ...
  if (node.wildcard != null) {
    ...
    if (wildcardMethods != null) {
      ...
      str += node.methods == null ? `if(l>${currentIdx}){${compiled}}` : compiled;
    }
  }
 ...
 * @param deps - Dependencies of the function scope
 */
export const _compileRouteMatch = (router: RouterContext<any>, deps: any[]): string => {
  let str = '';
 ...
 * @param router
 */
export const compileRoute = <T>(router: RouterContext<T>): (path: string, method: string) => MatchedRoute<T> | undefined => {
  const deps: any[] = [];
  const compiled = _compileRouteMatch(router, deps);

  return new Function(...deps.map((_, i) => 'd' + (i + 1)), `return(p,m)=>{${compiled}}`)(...deps);
}
