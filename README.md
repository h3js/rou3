...
The regex matches **exactly** the paths `findRoute()` matches for a router holding only that route, so it is safe to use as a guard or scope check in place of the router. That includes the router's lookup tolerances: up to two trailing slashes (`/users/123//`), empty segments for whole-segment `:name` and `*` params (`/a//b` matches `/a/:x/b`), and an optional trailing `*` (`/a` matches `/a/*`). The trailing `(?:\/\/|(?<!\/)\/?)` encodes the slash rule: at most two trailing slashes, and exactly two when the path's last segment is empty. Paths are compared as-is, like `findRoute()` without `{ normalize: true }`, so resolve `.`/`..` segments first if the router normalizes them. The fixed-length look-behind is supported by JavaScript, PCRE and Perl, but not by RE2-based engines (Go, the Rust `regex` crate).
