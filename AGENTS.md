...
- **Trailing slashes:** every non-root route ends in `TRAILING_SLASHES` = `(?://|(?<!/)/?)$` — lookup strips up to two trailing `/`, and when the body ends in `/` (a real empty last segment, e.g. `/a///` → `/a/:x` with `x: ""`) exactly two must follow. Root stays `^\/\/?$`. The look-behind is fixed-length (PCRE/Perl-safe).
...
- Known residual: an empty segment directly before a wildcard (`/a//**`, `/a//*`; `/a//**:x` agrees) still has the regex over-matching relative to the tree — the `**` emission leaves the empty segment's separator in place but makes the catch-all optional (`\/(?:\/…)?`, so `/a/` and `/a//` match), and lookup-side trailing normalization erases the empty segment in that position anyway. Pathological; not modeled.
