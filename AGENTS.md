...
test/
  ...
  regexp.pcre.test.ts # Cross-engine checks: PCRE (grep -P/rg -P/pcre2grep/perl/php) + RE2-family (rg default engine = Rust regex)
  _regexp-cases.ts    # Shared route->regex fixtures + sweep corpus (used by regexp.test.ts + regexp.pcre.test.ts)
...
`routeToRegExp(p).test(path)` must agree with `findRoute(router_with_only_p, "", path) !== undefined` — consumers use the regex as a guard/scope check (h3 v2 `use(route, mw)` inherited an auth bypass from an under-match), so the regex mirrors the **trie's** tolerances rather than URLPattern's. Pinned by the property sweep in `test/regexp.test.ts` ("matches exactly the paths findRoute matches") plus `_regexp-cases.ts` fixtures; the 7 `/foo/`-style WPT cases moved from `ROUTER_KNOWN_DIFFS` to `KNOWN_DIFFS` accordingly. The rules:

- **Trailing slash:** lookup strips at most one trailing `/` (#209), so a path matches iff the body matches it with one trailing `/` stripped; when the body match ends in `/` (a real empty last segment, e.g. `/a//` → `/a/:x` with `x: ""`) exactly one more must follow. Root is `^\/$`. `withTrailingSlash()` (`src/_trailing-slash.ts`) appends the rule **look-behind free where possible (RE2/Go/Rust `regex`-compatible)**, by tokenizing the emitted body (one atom regex + depth counter; class-aware because a constraint like `[(]` would otherwise break depth) and checking which of the last segment / trailing optional inners can match empty (`canBeEmpty`, a `RegExp` probe — needed for user constraints): body can't end in `/` → `\/?$`; only the last trailing `(?:/X)?` can be empty → `\/?$` with that group lazy `??` (and `.*` → `.*?`) so captures match the router (`**`'s `_` stays greedy-group: the router reports `""` on `/a/`); required whole `:x` → `(?:(?<x>[^/]+)\/?|\/)$`; `**:x`/`:x+` → `(?:\/|(?<x>.+?)\/?)$`; root `/**` → `^\/?(?<_>.*?)\/?$`. The two required forms leave the group **unset** on the empty-last-segment path where the router reports `""` — inherent: without look-around, excluding `/a/` while capturing `""` on `/a//` needs the group in two branches (duplicate names, which RE2 also rejects). Everything else keeps the fixed-length look-behind `(?:(?<=/)/|(?<!/)/?)$` (PCRE/Perl-safe, not RE2): a required empty-capable segment followed by optionals (`/a/:x/:y?`, provably impossible without dup names), empty-capable constraints (`:x(\d*)`), an empty segment before a trailing wildcard (`/a//*`), several empty-capable optionals in a row (making earlier ones lazy shifts values into later groups), an optional group spanning segments whose last one can be empty (`/a{/b/:x}?`: its inner can't be empty but can end in `/`, and `/a/b/` strips to `/a/b`, which matches neither branch). Tracked in `LOOKBEHIND_ROUTES` (`_regexp-cases.ts`, asserted both ways); the RE2 suite in `regexp.pcre.test.ts` runs every fixture and every look-behind-free sweep pattern through `rg` without `-P`. Captures were checked against the router over the whole sweep: the unset-vs-`""` case is the only regression, and the lazy groups fixed 38 prior mismatches. `regExpToRoute` normalizes the new endings back to the plain forms before parsing, and still accepts the look-behind suffix and the pre-#209 two-slash suffix `(?:\/\/|(?<!\/)\/?)`.
...
- **Trailing `*` is optional** (`(?:/(?<_N>[^/]*))?`), also when only `?`/`*`-modified segments follow it.
- **`**:name` / `:name+` → `/(?<n>.*)`** (required separator, segments may be empty); `.+` let `/a//` match with `n="/"`.
- **Router-order expansion (`needsModifierExpansion`):** a mid-route `:x+` / `:x*` (`/a/:x+/b` is `/a/**:x` in the tree) and an empty segment followed only by optional ones (`/docs/{v2}?/:page?` → `/docs//:page?` also registers `/docs`, the empty segment turning trailing) expand through the router's own `expandModifiers`, *after* group expansion like `addRoute`. `inlineOptionalGroup` bails on such routes (it used to feed the unexpanded prefix to the emitter: `/api/:p+/meta{.json}?` missed `/api/secret`). Identical branch sources are deduped before joining.
- A root-level optional is emitted as `(?:/X)?` with no leading `/`: `routeToRegExpSegments` returns `[segments, ownSeparator]` and `joinSegments` drops the leading `/` on that flag (an in-band sentinel char collided with literal route content, e.g. a NUL). The old `^\/?X` form let the leading slash double as a trailing one.
- **Not modeled:** constraints that can match `/` (`:x(.+)`, `:x(.*)?`) — the tree splits on `/` first, the regex doesn't, so `/a/:x(.+)` matches `/a/b/c` in the regex; the look-behind-free `\/?$` ending also lets such a constraint absorb a trailing slash (`/a//`), which an empty-string probe can't detect; repeated constrained params (`:id(\d+)+` / `*`) — a router bug: `expandModifiers` drops the constraint in the tree while the regex keeps it (regex under-matches); the empty path `""` (the router matches it against root-reaching routes, the regex requires a leading `/`); `normalize: true` lookups (`.`/`..` are resolved before matching — the regex sees the raw path).

### Catch-all regex emission

The separator before a catch-all is **never** a bare optional `\/?` after a prefix: `^\/api\/?(?<_>.*)` matched `/apifoo`, and `.+` could capture the separator itself (`/base/**:p` on `/base/` gave `p="/"`). Now `/api/**` → `^\/api(?:\/(?<_>.*))?<TRAILING_SLASH>` (zero or more segments; on `/api` and `/api/` the group is unset where the router reports `""`), `/base/**:p` → `^\/base\/(?<p>.*)<TRAILING_SLASH>`, and root `/:x+` / `/**:x` → `^\/(?<x>.*)<TRAILING_SLASH>`. Only root `/**` keeps `^\/?(?<_>.*)` (no prefix to overrun). Side effect for `regExpToRoute`: `**:name` and `:name+` now emit **byte-identical** regexes (both one-or-more segments, `compareRoutes` → `equal`, same node key), so `**:name` reverses to `:name+`; likewise `/a/:_*` and `/a/**` share output and a *trailing* `(?:\/(?<_>.*))?` reverses to `**`. Source round-trip still holds. `_regexp-cases.ts` fixtures carry `noMatch` paths asserted against the tree, the JS regex and every PCRE engine — negative cases are what was missing when this shipped.
...
- Targets the dialect `routeToRegExp()` emits. The parser strips `^`/`$` and the trailing-slash suffix (`(?:(?<=\/)\/|(?<!\/)\/?)`, or the legacy pre-#209 `(?:\/\/|(?<!\/)\/?)` / `\/?`), then walks the body recognizing: static separators (`\/`) + segments, catch-alls (a trailing `(?:\/(?<_>.*))?` → `**`; the legacy `\/?(?<_>.*)` / `\/?(?<name>.+)` forms → `**` / `**:name` — still emitted for a root `/**`, and accepted from older rou3 output), and optional-group units (`(?:\/…)?`). A trailing `\/(?<name>.*)` → `:name+` (so `/a/:x(.*)` reverses to `/a/:x+` — byte-identical regex). Segment-internal parsing maps `(?<name>[^/]*)` / legacy `(?<name>[^/]+)` → `:name`, optional `(?:\/(?<_N>[^/]*))?` → `*`, `(?<_N>[^/]*)` / bare `([^/]*)` → `*`, `(?<_N>pat)` / bare `(pat)` → `(pat)`, `(?<name>pat)` → `:name(pat)`, and re-escapes literal route-syntax chars (`: ( ) { } * \`). Bare (unnamed) capturing groups map like their `(?<_N>…)` counterparts.
