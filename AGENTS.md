...
- Targets the dialect `routeToRegExp()` emits. The parser strips `^`/`$` and the trailing `\/?`, then walks the body recognizing: static separators (`\/`) + segments, catch-alls (a trailing `(?:\/(?<_>.*))?` → `**`; the legacy `\/?(?<_>.*)` / `\/?(?<name>.+)` forms → `**` / `**:name` — still emitted for a root `/**`, and accepted from older rou3 output), and optional-group units (`(?:\/…)?`). Segment-internal parsing maps `(?<name>[^/]+)` → `:name`, `(?<_N>[^/]*)` / bare `([^/]*)` → `*`, `(?<_N>pat)` / bare `(pat)` → `(pat)`, `(?<name>pat)` → `:name(pat)`, and re-escapes literal route-syntax chars (`: ( ) { } * \`). Bare (unnamed) capturing groups map like their `(?<_N>…)` counterparts.
